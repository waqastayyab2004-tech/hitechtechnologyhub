...
function OrgNode({ label, top = false, wide = false, activeNode, setActiveNode }: {
  label: string; top?: boolean; wide?: boolean;
  activeNode: string | null; setActiveNode: (v: string | null) => void
}) {
  const isActive = activeNode === label
  return (
    <button
      onClick={() => setActiveNode(isActive ? null : label)}
      className={`w-full px-3 py-3 rounded-xl text-white font-bold text-center leading-snug transition-all duration-150 border-2 ${
        isActive ? 'border-cyan-400 scale-105 shadow-[0_0_20px_rgba(34,211,238,0.4)]' : 'border-transparent hover:border-blue-400/50 hover:scale-105'
      } ${wide ? 'col-span-2' : ''}`}
      style={{
        background: top ? 'linear-gradient(135deg,#3b82f6,#1d4ed8)' : isActive ? 'linear-gradient(135deg,#0e7490,#1e40af)' : 'linear-gradient(135deg,#1e40af,#1e3a8a)',
          ...
          <>
            <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-[#0a1628] to-[#0d2040] p-8 overflow-x-auto mb-4">
              <div className="min-w-[640px]">
                <div className="flex justify-center mb-4"><OrgNode label="CIO" top activeNode={activeNode} setActiveNode={setActiveNode} /></div>
                <div className="flex justify-center mb-4"><div className="w-0.5 h-6 bg-blue-500/50"/></div>
                <div className="flex justify-center mb-4"><OrgNode label="VP of IT" top activeNode={activeNode} setActiveNode={setActiveNode} /></div>
                ...
                <div className="grid grid-cols-4 gap-3 mb-1">
                  {['Applications & Systems Director','IT Operations Director','Security & Compliance Director','IT Strategy & Architecture'].map(t => <OrgNode key={t} label={t} activeNode={activeNode} setActiveNode={setActiveNode}/>)}
                </div>
                <div className="grid grid-cols-4 gap-3 mb-1">
                  {[0,1,2,3].map(i => <div key={i} className="flex justify-center"><div className="w-0.5 h-4 bg-blue-500/40"/></div>)}
                </div>
                <div className="grid grid-cols-4 gap-3 mb-1">
                  {['Business Apps Manager','Infra Manager','Security Ops Manager','Enterprise Architect'].map(t => <OrgNode key={t} label={t} activeNode={activeNode} setActiveNode={setActiveNode}/>)}
                </div>
                <div className="grid grid-cols-4 gap-3 mb-1">
                ...
                <div className="grid grid-cols-4 gap-3">
                  <OrgNode label="ERP/CRP Admin" activeNode={activeNode} setActiveNode={setActiveNode}/>
                  <OrgNode label="Sys Admin & Infra Engineer" activeNode={activeNode} setActiveNode={setActiveNode}/>
                  <OrgNode label="Help Desk / IT Support Center" wide activeNode={activeNode} setActiveNode={setActiveNode}/>
                </div>
              </div>
            </div>
            {activeNode && ORG[activeNode] && (
              <div className="mb-6 rounded-2xl border border-cyan-500/30 bg-gradient-to-br from-[#0e1e38] to-[#0a1628] p-6">
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div>
                    <h3 className="text-lg font-black text-white">{ORG[activeNode].title}</h3>
                    <p className="text-xs text-cyan-400 font-semibold mt-0.5">{ORG[activeNode].reports}</p>
                  </div>
                  <button onClick={() => setActiveNode(null)} className="text-gray-500 hover:text-white text-xl leading-none flex-shrink-0">&#x2715;</button>
                </div>
                <p className="text-gray-300 text-sm leading-relaxed mb-4">{ORG[activeNode].focus}</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs font-black uppercase tracking-widest text-blue-400 mb-2">Tools &amp; Platforms</p>
                    <div className="flex flex-wrap gap-1.5">
                      {ORG[activeNode].tools.map(t => <span key={t} className="px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-500/15 border border-blue-500/25 text-blue-300">{t}</span>)}
                    </div>
                  </div>
                  <div>
                    <p className="text-xs font-black uppercase tracking-widest text-cyan-400 mb-2">Key Skills</p>
                    <div className="flex flex-wrap gap-1.5">
                      {ORG[activeNode].skills.map(s => <span key={s} className="px-2 py-0.5 rounded-full text-xs font-semibold bg-cyan-500/10 border border-cyan-500/20 text-cyan-300">{s}</span>)}
                    </div>
                  </div>
                </div>
              </div>
