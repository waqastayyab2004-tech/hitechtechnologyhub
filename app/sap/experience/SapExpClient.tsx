...
export default function SapExpClient() {
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      {/* Header */}
      <div className="bg-gradient-to-br from-indigo-700 via-indigo-800 to-blue-900 text-white pt-24 pb-12">
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex items-center gap-2 mb-3">
            <Link href="/sap" className="text-indigo-200 hover:text-white text-sm transition-colors">SAP Hub</Link>
          ...
          <p className="text-indigo-100 text-lg">11+ years at SAP Saudi Arabia across two progressive roles — IT Administrator to Senior IT System Engineer & SPOC</p>
        </div>
      </div>
        ...
        <main className="flex-1 min-w-0">
          <motion.div initial="hidden" animate="show" variants={stagger}>
            ...
            <motion.div variants={fade} className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl p-6 mb-10 flex items-start gap-4">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest text-blue-200 mb-1">Key Responsibility</p>
                  ...
                  {roles.map((role, i) => (
                    <motion.div key={i} variants={fade} className="sm:pl-14 relative">
                      <div className={`hidden sm:flex absolute left-3 top-6 w-5 h-5 rounded-full items-center justify-center
                        ${role.current ? 'bg-blue-600 ring-4 ring-blue-100 dark:ring-blue-950' : 'bg-slate-400 dark:bg-slate-600'}`}>
                        {role.current && <span className="w-2 h-2 rounded-full bg-white" />}
                      </div>

                      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 hover:shadow-md transition-shadow">
                        <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              {role.current && (
                                <span className="bg-green-100 dark:bg-green-950 text-green-700 dark:text-green-400 text-xs font-semibold px-2.5 py-0.5 rounded-full">
                                  Current
                                </span>
                              )}
                            </div>
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white">{role.title}</h3>
                            <p className="text-blue-600 dark:text-blue-400 font-semibold text-sm mt-0.5">SAP</p>
                          </div>
                          <div className="text-right shrink-0">
                            <p className="text-sm font-medium text-slate-700 dark:text-slate-300">{role.period}</p>
            ...
            <motion.div variants={fade}>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-6">LinkedIn Recommendations</h2>
              <div className="grid md:grid-cols-2 gap-5">
                {recommendations.map((r) => (
                  <div key={r.name} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
                    <Quote className="w-8 h-8 text-blue-100 dark:text-blue-900 mb-3" />
                    <p className="text-slate-700 dark:text-slate-300 text-sm leading-relaxed mb-5">&ldquo;{r.text}&rdquo;</p>
                    <div className="border-t border-slate-100 dark:border-slate-800 pt-4">
                      <p className="font-bold text-slate-900 dark:text-white text-sm">{r.name}</p>
                      <p className="text-slate-500 dark:text-slate-400 text-xs mt-0.5 leading-relaxed">{r.title}</p>
                      <p className="text-slate-400 text-xs mt-1">{r.date} · {r.relationship}</p>
                    </div>
                  </div>
                ))}
              </div>
