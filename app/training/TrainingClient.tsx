...
const courses = [
  /* ── CORPORATE IT TRAINING (from SAP Litmos) ─────────────────── */
  {
    ...
    skills: ['Asset Tracking', 'Hardware Lifecycle', 'enterprise asset management system', 'MDM Compliance', 'Inventory Management'],
    description: 'Learn how enterprise IT assets are tracked, returned, scanned, and audited across the full hardware lifecycle — from procurement to disposal. Based on real enterprise IT operations.',
    topics: [
      ...
      'Asset records management in enterprise ERP (enterprise asset management system)',
    ],
    curriculum: [
      { module: 'Module 1 — Asset Management Fundamentals', lessons: ['What is IT asset lifecycle management?', 'Enterprise asset categories: laptops, mobiles, tablets, monitors, printers', 'Asset tagging, serial number tracking, and enterprise asset management system records', 'Minimum device lifespan policies and refresh cycles'] },
  ...
  },
  {
    ...
    description: 'Configure, enrol, and support corporate Macs using Jamf Pro — covering ADE (Automated Device Enrolment) for zero-touch setup, manual DE for exception cases, Setup Assistant configuration, FileVault encryption, Kerberos SSO, VPN and Conditional Access registration, Self Service app portal, compliance enforcement, and Recovery Mode troubleshooting. Based on managing 50+ Macs with ~95% ADE coverage.',
    topics: [
      'ADE vs. DE enrolment: what each is, when each applies, and how to execute both',
      'Setup Assistant: pre-requisites (MFA/TAP), account creation with corporate ID and password',
      'Post-setup: GlobalProtect VPN, Microsoft 365, and Conditional Access registration',
      'FileVault encryption and Kerberos SSO: enforcement via Jamf and daily troubleshooting',
      'macOS troubleshooting: Recovery Mode reinstall, T2 chip Secure Boot, failed upgrades',
    ],
    curriculum: [
      { module: 'Module 1 — Corporate Mac Enrolment: ADE vs. DE', lessons: ['What is Jamf Pro and how it manages the corporate Mac fleet', 'ADE (Automated Device Enrolment): ~95% of Macs — auto-enrolled at first boot via serial number registration', 'DE (Manual Device Enrolment): ~5% — exception cases where device is not registered in management system', 'BYOD personal Macs: NOT permitted — only corporate-owned devices can be enrolled', 'Pre-requisite before any Mac setup: MFA configured OR Temporary Access Pass (TAP) issued to user', 'How to execute ADE: power on → connect to Wi-Fi → Setup Assistant runs → MDM profile auto-installed'] },
      { module: 'Module 2 — Setup Assistant & Account Configuration', lessons: ['Setup Assistant: the guided first-boot configuration wizard — run by the user, not IT', 'Account creation rules: Full Name = Last, First | Account name = Corporate ID (I/C/D#) | Password = Corporate global password', 'Manual DE enrolment: download MDM profile → install → verify enrolment in System Settings → Management profile active', 'What happens after enrolment: Jamf policies push, corporate apps install, FileVault activates', 'MFA/TAP: why it must be set up BEFORE Mac setup — troubleshooting if missed', 'Verifying successful enrolment: Jamf Self Service installed, MDM profile visible, FileVault enabled'] },
      { module: 'Module 3 — Post-Setup: VPN, M365 & Conditional Access', lessons: ['GlobalProtect VPN (Cisco Secure Client): install from Self Service → connect → verify corporate network access', 'Microsoft 365: sign in with corporate credentials — Conditional Access checks device compliance before access granted', 'Conditional Access (CA) registration: required for Outlook, Teams, OneDrive to work on Mac', 'How to register macOS with Conditional Access step by step', 'FileVault: enforced via Jamf policy — recovery key automatically escrowed, user receives notification', 'Kerberos SSO certificate: installed via Jamf → syncs corporate password to macOS local account → resolves password mismatch tickets'] },
      { module: 'Module 4 — Jamf Self Service & Daily Mac Support', lessons: ['Self Service app portal: 40+ corporate apps available to users without IT ticket', 'How to install software from Self Service: open app → browse → click Install', 'Requesting software not in Self Service: raise ITSM ticket → IT evaluates → deploys via Jamf policy', 'macOS mandatory update policy: 30-day window post Apple release — Jamf enforces, user prompted daily', 'Privileges app: request temporary local admin rights — time-limited, audited in Jamf', 'Compliance monitoring: Jamf dashboard shows non-compliant Macs — daily check and remediation'] },
      { module: 'Module 5 — Troubleshooting macOS: Recovery Mode & Common Issues', lessons: ['Recovery Mode: when and how to enter (hold Cmd+R at startup) — used for reinstall and security changes', 'Reinstall macOS via Recovery Mode: internet recovery, clean install procedure step by step', 'T2-chip Macs (mid-2018+): cannot boot from USB by default — always use Recovery Mode, not external media', 'Secure Boot options on T2 Macs: changing is against security compliance — do not lower security settings', 'DEP/ADE exception: Macs not under ADE cannot be added retroactively — only Apple OEM/Apple can add to DEP', 'Common ticket: Kerberos SSO broken after password change — solution: re-enter corporate password in SSO extension', 'Common ticket: macOS upgrade failed, Mac won\'t boot — solution: Recovery Mode reinstall, data preserved if FileVault intact'] },
    ],
  },
  {
    id: 104,
  ...
  },
  {
    ...
    description: 'Understand corporate network types, configure VPN remote access, connect managed devices to wireless, and apply DNS-layer security — based on real enterprise MENA infrastructure.',
    topics: [
  ...
  },
  {
    ...
    curriculum: [
      { module: 'Module 1 — Enterprise Print Infrastructure', lessons: ['HP MFP fleet overview: device types, locations, and naming conventions', 'WebJet Admin: fleet monitoring dashboard and alert configuration', 'Print server architecture: how jobs are routed to the correct device', 'badge reader integration: authentication flow', 'Global Print Services (GPS) support model and escalation path'] },
  ...
  },
  {
    ...
    isFree: true, isPopular: false,
    img: 'https://images.unsplash.com/photo-1555949963-aa79dcee981c?w=400&q=80&auto=format&fit=crop',
    instructor: 'Syed Waqas Tayyab',
  ...
  },
  {
    ...
    instructor: 'Syed Waqas Tayyab',
    skills: ['Jamf MDM', 'ServiceNow Walk-up', 'Digital Signage', 'Apple ADE', 'Microsoft Intune', 'Queue Management', 'corporate password vault'],
    ...
    curriculum: [
      { module: 'Module 1 — What is ITLC & System Architecture', lessons: ['What is an IT Link Center (ITLC) and its role in enterprise IT support', 'Two-component system: iPad Check-in Kiosk + Queuing Monitor display', 'ServiceNow Walk-up module: how it improves queue management', 'Network requirements: corporate guest Wi-Fi for setup, corporate Wi-Fi for operation', 'Prerequisites: iPad 10"+ with iPadOS 15+, Jamf Cloud ADE enrolment'] },
      { module: 'Module 2 — iPad Kiosk Setup & MDM Enrolment', lessons: ['Out-of-box iPad setup: language, region, manual setup selection', 'Connect to corporate guest Wi-Fi and begin Jamf ADE enrolment', 'Naming convention: ITLC-[building code] — why it matters', 'Required settings: brightness max, sound off, Bluetooth off, Light mode', 'Connecting to corporate Wi-Fi and saving the Walk-up portal URL in Safari'] },
      ...
      { module: 'Module 4 — Service Account Login via corporate password vault', lessons: ['Two Walk-up service accounts: walk-up service account (English) and walk-up service account (French)', 'corporate password vault: what it is and how to access it for the first time', 'Retrieving the Walk-up account password from corporate password vault step by step', 'Logging into the ServiceNow Walk-up portal and selecting your location', 'Security note: always retrieve passwords from corporate password vault — not from stored documents'] },
  ...
  },
  {
    ...
    topics: [
      ...
      'On-boarding workflow: assign pre-staged devices, link to ServiceNow and enterprise asset management system',
    ...
    curriculum: [
      { module: 'Module 1 — CLEA App Overview & Architecture', lessons: [
        ...
        'How CLEA connects to enterprise asset management system, ServiceNow ITSM, SAP Ariba, and Power BI',
      ...
      { module: 'Module 2 — Monthly Pool Stock Scan (IT Storage Room Inventory)', lessons: [
        ...
        'What happens in the backend: enterprise asset management system records updated, Power BI reflects new data',
      ]},
      { module: 'Module 3 — On-Boarding: Device Assignment for New Joiners', lessons: [
        ...
        'Verifying the assignment appears in enterprise asset management system asset record',
      ]},
      ...
      { module: 'Module 5 — Power BI Asset Reporting & KPI Tracking', lessons: [
        ...
        'Missing Data report: how to identify assets with incomplete enterprise asset management system records',
  ...
  {
    id: 116,
    title: 'ServiceNow CSM/FSM Workspace & ITSM AI Copilot for IT Support',
    category: 'IT Ticketing & ITSM',
    level: 'Intermediate', duration: '2h 30m', students: '95+', rating: 4.9,
    isFree: true, isPopular: true,
    img: '/snow-ticketing-banner.jpg',
    instructor: 'Syed Waqas Tayyab',
    skills: ['ServiceNow CSM/FSM', 'ITSM AI Copilot', 'Walk-up Queue', 'Interaction Management', 'Catalog Tasks', 'SLA IRT/MPT', 'ITIL v3'],
    description: 'Master the ServiceNow CSM/FSM Configurable Workspace for daily IT support operations — managing interactions, catalog tasks, incidents, walk-up queue, and using the ITSM AI Copilot for faster resolution. Based on real daily operations handling 1,300+ interactions and 65+ catalog tasks per month.',
    topics: [
      ...
      'ITSM AI Copilot: triggering it, reading suggestions, applying use cases in tickets',
    ...
    curriculum: [
      ...
      { module: 'Module 4 — ITSM AI Copilot in Daily Operations', lessons: [
        'What is ITSM AI Copilot and where it lives in the CSM/FSM workspace',
  ...
  {
    id: 117,
    title: 'Office AV Technology: Plan, Install & Commission Meeting Room Tech',
    category: 'Office & AV Technology',
    level: 'Intermediate', duration: '3h', students: '75+', rating: 4.9,
    isFree: true, isPopular: false,
    img: 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=400&q=80&auto=format&fit=crop',
    instructor: 'Syed Waqas Tayyab',
    skills: ['MS Teams Rooms', 'AV Over IP', 'Room Schedulers', 'Wolfvision Cynap Pure', 'Digital Signage', 'Display Technologies', 'Audio Systems', 'Office Build-Out'],
    description: 'Learn how to plan, install, configure, and commission enterprise office AV technology — covering meeting room standards, Microsoft Teams Rooms, display technologies, wireless presentation (Wolfvision Cynap Pure), room schedulers, audio systems, AV-over-IP, and digital signage. Based on real office build-out delivery managing 15+ meeting rooms.',
    topics: [
      'Meeting room technology standards: approved tech stack per room category (huddle to VIP)',
      'Microsoft Teams Rooms: certified hardware selection, installation, and configuration',
      'Display technologies: projectors, LED video walls, interactive displays, mounts',
      'Wireless presentation (Wolfvision Cynap Pure): setup, network integration, BYOD config',
      'Room schedulers (eDoorplates): Crestron/Evoko setup and Exchange/Teams integration',
    ],
    curriculum: [
      { module: 'Module 1 — Meeting Room Standards & Technology Selection', lessons: [
        'Meeting room categories: XS huddle (1-4), Small/Med (5-10), Large (10-20), Boardroom/VIP (20+)',
        'Technology matrix: approved AV stack per room category — what goes where and why',
        'Microsoft Teams Rooms certified hardware: bars, cameras, audio, compute units',
        'Display selection: screen size formula by room depth, throw ratio for projectors',
        'AV Over IP vs. traditional HDMI distribution: when to use each',
        'Building a Bill of Materials (BOM) before procurement — what must be on the list',
      ]},
      { module: 'Module 2 — Microsoft Teams Rooms Deployment', lessons: [
        'Teams Rooms hardware setup: unboxing, mounting, cable management',
        'Initial configuration: Teams Rooms Pro licence, account setup, domain join',
        'One-touch meeting join: verifying calendar integration with Exchange/M365',
        'Teams Rooms Admin Centre: monitoring room health, app version, peripheral status',
        'Nightly health check: what to look for and how to action amber/red status',
        'Common Teams Rooms issues and on-site remediation: camera, audio, display faults',
      ]},
      { module: 'Module 3 — Display & Audio Technologies', lessons: [
        'Display types: LCD panels, interactive flat panels, LED video walls, projectors',
        'Display mounts and trolleys: fixed wall, flush, motorised, and mobile configurations',
        'Digital flipcharts: interactive whiteboard setup and network integration',
        'Audio design: ceiling mic coverage zones, table boundary mics, gooseneck placement',
        'Handheld and lavalier microphones: wireless frequency management, charging docks',
        'Loud speaker placement: coverage calculation, delay zones, feedback prevention',
        'AV calibration: audio levels, display brightness/colour profile standardisation',
      ]},
      { module: 'Module 4 — Wireless Presentation & Room Schedulers', lessons: [
        'Wolfvision Cynap Pure: what it is and why it replaces HDMI cables in meeting rooms',
        'Cynap Pure installation: network connection, vSolution Link Pro management portal',
        'BYOD wireless presentation: connecting Windows, macOS, iOS, Android without cables',
        'Cynap Pure troubleshooting: connectivity issues, firmware updates, factory reset',
        'Room schedulers (eDoorplates): Crestron vs. Evoko — feature comparison',
        'Doorplate setup: physical installation, network join, Exchange/Teams calendar binding',
        'Booking panel behaviour: check-in, ad-hoc booking, auto-release after 10 min no-show',
      ]},
      { module: 'Module 5 — Digital Signage, Build-Out Delivery & Handover', lessons: [
        'Digital signage architecture: content management platform, display network, scheduling',
        'Digital Signage portal: registering screens, creating playlists, setting display duration',
        'AV Over IP distribution: encoder/decoder setup, network VLAN requirements',
        'Office build-out checklist: from floor plan review to site acceptance sign-off',
        'Vendor management: scoping AV contractor work, quality checks during installation',
        'Site acceptance testing: AV, network, booking, wireless — formal sign-off procedure',
        'Handover documentation: network diagram, AV config sheet, warranty register, support contacts',
      ]},
    ],
  },
  {
    id: 118,
    title: 'IT Pickup Point: Deploy & Manage Smart Lockers for Self-Service IT Asset Collection',
    category: 'Office & AV Technology',
    level: 'Intermediate', duration: '2h', students: '55+', rating: 4.9,
    isFree: true, isPopular: false,
    img: '/it-pickup-locker-2.jpg',
    instructor: 'Syed Waqas Tayyab',
    skills: ['Signifi Digital Locker', 'Smart Locker Management', 'IT Asset Automation', 'SignifiVISION', 'Self-Service IT', 'Audit Trail', 'ITAM Integration'],
    description: 'Learn to deploy and operate a Signifi Digital Storage Locker IT Pickup Point — from the 11-point pre-installation readiness checklist through to daily operations. Covers the complete delivery status lifecycle, step-by-step locker and ITLC pickup procedures, 7-day expiry policy, kiosk inventory reporting, emergency key protocol, and deployment cost management. Based on a real production deployment.',
    topics: [
      'What is a smart IT locker and how Signifi Digital Storage Lockers work',
      'Deployment planning: placement, network, power, and branding',
      'SignifiVISION software: setup, compartment configuration, user authentication',
      'Integration with asset management: procurement → locker → collection → record update',
      'Daily operations: loading, notifications, audit trail, and issue resolution',
    ],
    curriculum: [
      { module: 'Module 1 — Smart Locker Fundamentals & Delivery Workflow', lessons: [
        'What is a Signifi digital storage locker and how it replaces manual IT asset handoff',
        'Delivery status lifecycle: Pending Technician Drop-off → Pending Customer Pickup → Complete',
        'Two pickup channels: Smart Locker (self-service QR/PIN) vs. IT Link Center (staff-assisted)',
        'When to use ITLC instead of locker: item too large, employee requested it, or locker unavailable',
        'Signifi at scale: 350M+ transactions/year, 45+ countries, 85+ API integrations',
        'Compliance advantage: authenticated digital audit trail eliminates sign-out sheets permanently',
      ]},
      { module: 'Module 2 — Create a Locker Pickup (Step-by-Step)', lessons: [
        'Step 1: Access the locker management application — Manage Deliveries page shows recent history',
        'Step 2: Select "Deliver Order" → enter the order/PO number from the shipping label or packing slip',
        'Step 3: Select Delivery Place from dropdown → choose the correct locker location',
        'Step 4: Select the compartment size — small (phones/accessories), medium (tablets), large (laptops)',
        'Step 5: Submit and confirm → status changes to "Pending Technician Drop-off"',
        'Step 6 (at the locker): Enter 6-digit technician PIN → select item from screen → door opens → insert item → close and confirm locked',
        'Step 7: Status changes to "Pending Customer Pickup" → employee automatically receives QR code + PIN by email',
      ]},
      { module: 'Module 3 — IT Link Center (ITLC) Pickup & 7-Day Expiry Policy', lessons: [
        'Creating an ITLC pickup: select ITLC as Delivery Place → submit → optionally trigger notification email',
        'Marking ITLC collection complete: click "Picked Up" → confirm → confirmation email sent to employee',
        'Complete without email: use when item already collected but not yet marked in system (Actions → Complete → Without email)',
        '7-day expiry policy: items uncollected after 7 days must be moved from locker to ITLC to free space',
        'Moving an expired locker item: log in with PIN → select expired item → door opens → remove → create new ITLC record',
        'Cancelling an entry: Actions → Cancel → only available for items in "Pending Customer Pickup" status, before item is physically placed',
        'Multiple boxes with same PO: if arriving separately use different lockers; use Description field to differentiate shipments',
      ]},
      { module: 'Module 4 — Data Management & Reporting', lessons: [
        'Kiosk Inventory Status report: paste the full locker name → view all pending deliveries at that site',
        'Export Deliveries to Excel: covers Pending Technician Drop-off, Pending Customer Pickup, and Expired statuses',
        'Report covers both IT Pickup Points and IT Link Center records in one view',
        'Handling wrong locker size: log in with PIN → do tech drop-off → when door opens → tap "Issue with locker" → "Choosing another locker" → confirm',
        'Emergency key protocol: unlocks ALL doors in a locker set — use when compartment stuck or wrong item placed',
        'Locker screen timeout: screen stays active for 30 seconds after login — act promptly',
        'Data export limitation: date filter on timestamp fields does not filter results — all matching records are shown',
      ]},
      { module: 'Module 5 — Deployment Readiness & Pre-Installation Checklist', lessons: [
        '11-point Signifi deployment readiness checklist: must be completed and returned to PM before installation',
        'Site access: COI and safe permits / work permits for third-party equipment entry — submit and approve in advance',
        'Power: verify outlet type, voltage, and position near planned locker location',
        'Network: MAC address whitelisting, LAN port available, IP address assignment confirmed with IT team',
        'Physical logistics: elevator/dock booked, delivery path clear, floor level and potential need for lifting equipment',
        'IT team on standby: escalation contact confirmed for installation day troubleshooting',
        'Out-of-scope fees: abort on arrival ($500), short-notice cancellation ($250), extended troubleshooting ($150/hr) — confirm readiness to avoid',
        '30-day Signifi warranty: all early-life failures and workmanship faults covered at no additional cost',
      ]},
    ],
  },
  {
    id: 119,
    title: 'Zebra ZT411R RFID Printer: Full Setup, Calibration & Asset Tag Printing',
    category: 'Printer Management',
    level: 'Intermediate', duration: '2h', students: '40+', rating: 4.9,
    isFree: true, isPopular: true,
    img: '/zebra-zt411-home-screen-idle.webp',
    instructor: 'Syed Waqas Tayyab',
    skills: ['Zebra ZT411R', 'RFID Configuration', 'ZebraNet Print Server', 'Label Media Loading', 'EU RED Compliance', 'Zebra Setup Utilities', 'Network Printing', 'CLEA App', 'IT Asset Management'],
    description: 'Complete end-to-end course on deploying and operating the Zebra ZT411R industrial RFID printer — from unboxing and media loading through manual media calibration, RFID calibration, EU RED security activation via Zebra Setup Utilities, wired network registration, ZebraNet print server configuration, CLEA integration, and daily asset tag operations. Based on a real production deployment.',
    topics: [
      'Zebra ZT411R hardware overview and RFID encoder fundamentals',
      'Loading RFID inlay media and thermal transfer ribbon',
      'Manual media calibration (Pause + Cancel method)',
      'RFID calibration via front panel — country selection and full cycle',
      'EU RED security activation using Zebra Setup Utilities + Security Setup File',
      'Network registration — DHCP, MAC address, Printer Team process',
      'ZebraNet print server configuration and verification',
      'CLEA integration and daily asset tag printing workflow',
      'Troubleshooting: VOID tags, label gaps, sensor issues, scanner keyboard mode',
    ],
    whatYouLearn: [
      'Unbox and install the Zebra ZT411R correctly — including placement for RFID accuracy',
      'Load RFID inlay media and thermal transfer ribbon without causing void tags',
      'Perform manual media calibration (Pause + Cancel) and verify single-label feed',
      'Run RFID calibration with correct regional settings — full 5-minute cycle',
      'Activate EU RED compliance by sending a Security Setup File via Zebra Setup Utilities',
      'Register the printer on the corporate network and obtain a stable DHCP reservation',
      'Configure ZebraNet print server and verify via the printer web interface',
      'Integrate with CLEA for real-time RFID asset record updates',
      'Diagnose and fix VOID labels, label gaps, QR code offsets, and scanner issues',
    ],
    curriculum: [
      { module: 'Module 1 — RFID Printer Fundamentals & Hardware Overview', lessons: [
        'What is RFID and why enterprise IT uses it for asset tracking',
        'Zebra ZT411R hardware tour: print engine, RFID encoder, ZebraNet print server, media path, ports',
        'Thermal transfer vs. direct thermal printing — why thermal transfer for durable RFID asset tags',
        'RFID inlay types: UHF vs. HF — enterprise IT standard is UHF (ISO 18000-6C / EPC Gen2)',
        'RFID label specification: 60×25mm Synthetic RFID Coated Acrylic, 400 labels/roll',
        'Unboxing checklist and placement guidelines: clearance, ventilation, away from UHF emitters',
      ]},
      { module: 'Module 2 — Hardware Installation & Media Loading', lessons: [
        'Ribbon loading: threading the ribbon path correctly, attaching to take-up spindle',
        'Why ribbon width must match label width — printhead wear and print edge quality',
        'Loading RFID inlay label rolls: positioning the inlay directly over the RFID antenna window',
        'Media guide adjustment: push firmly against label edges — loose guides cause calibration failures',
        'Power-on sequence and initial test print to confirm mechanical setup',
        'Reference videos: Ribbon and Label Position.MOV, Sensor label Adjust.MOV, Where the Sensor must light.MOV',
      ]},
      { module: 'Module 3 — Manual Media Calibration', lessons: [
        'Why manual calibration is required after every label roll change',
        'Step-by-step: Press and hold Pause + Cancel to enter manual calibration mode',
        'Why the ribbon must be removed during calibration — sensor reads label backing only',
        'Verifying calibration: press Feed and confirm exactly one label advances',
        'What causes multi-label feeds and how to repeat the calibration correctly',
        'Reference: Printer Calibration.docx and Sensor label Adjust.MOV',
      ]},
      { module: 'Module 4 — RFID Calibration & Encoder Setup', lessons: [
        'Why RFID calibration is separate from media calibration — inlay position varies per roll',
        'Front panel navigation: Menu → RFID Icon → RFID Calibrate → Start Calibration',
        'Selecting the correct country/region code — this affects RF output compliance',
        'Understanding calibration time: a full successful cycle takes more than 5 minutes',
        'If calibration finishes quickly — why this means it failed and how to retry',
        'Verify after calibration: print a test label and confirm RFID data reads back correctly',
      ]},
      { module: 'Module 5 — EU RED Security Activation via Zebra Setup Utilities', lessons: [
        'What EU RED is and why all RF-emitting equipment requires compliance activation',
        'Why this cannot be done from the front panel alone — requires Zebra Setup Utilities + Windows PC',
        'Download and install Zebra Printer Setup Utilities for Windows from zebra.com/support',
        'Connect the printer to your computer via USB Type-A to Type-B cable',
        'Select the printer in Zebra Setup Utilities and open Open Communication With Printer',
        'Paste the Security Setup File contents into the Direct Communication window and click Send to Printer',
        'Reading the response window: what a successful activation response looks like',
        'Setting Head Close Action to No Motion to prevent label waste on printhead open/close',
      ]},
      { module: 'Module 6 — Network Registration & Print Server Setup', lessons: [
        'Disable Power Save / Energy Star before network connection — prevents port deactivation by network team',
        'Connect via Ethernet (DHCP) — how to find the assigned IP address on the printer display',
        'Contact the Printer Team: what to provide — MAC address and current IP',
        'What the Printer Team does: hostname assignment, DHCP reservation, network directory registration',
        'Why print queues must wait until DHCP reservation is confirmed — silent IP change breaks all queues',
        'Accessing the ZebraNet print server web interface: Status READY, RFID READY, link status verification',
        'Print server pages: View/Modify Settings, RFID Log, Alert Setup, Directory Listing, Printer Controls',
        'SNMP alert configuration: Cold Start broadcast for network monitoring',
      ]},
      { module: 'Module 7 — CLEA Integration, Daily Operations & Troubleshooting', lessons: [
        'How RFID tags printed by the ZT411R are used in the CLEA asset lifecycle management system',
        'Daily tagging workflow: asset record → print → verify RFID → apply → update system',
        'Label quality check before applying: scan every tag — catch void tags before they reach a device',
        'Troubleshooting: VOID message on labels — RFID calibration not completed correctly',
        'Troubleshooting: label gap between prints — sensor position, run manual calibration',
        'Troubleshooting: QR code / barcode offset on label — adjust in label template code',
        'Troubleshooting: scanner cannot scan anything — set keyboard mode in Desktop123 software',
        'Ordering replacement labels: LABEL SYNTHETIC 60×25MM RFID COATED ACRYLIC 76.2MM 400/R',
      ]},
    ],
  },
  {
    id: 120,
    title: 'Zebra GX430t: QR Code Asset Tag Printing for Enterprise IT',
    category: 'Printer Management',
    level: 'Beginner', duration: '1h', students: '60+', rating: 4.8,
    isFree: true, isPopular: false,
    img: '/zebra-gx430t-printer.jpg',
    instructor: 'Syed Waqas Tayyab',
    skills: ['Zebra GX430t', 'QR Code Label Printing', 'Asset Tagging Workflow', 'CLEA App Scanning', 'IT Asset Management', 'Zebra Setup Utilities'],
    description: 'Learn to set up and operate the Zebra GX430t desktop label printer for enterprise IT asset tagging. Covers printer setup, label media loading, QR code template configuration, and the complete asset tagging workflow — from printing to mobile scanning with the CLEA app. Based on a real 3-year enterprise IT production deployment.',
    topics: [
      'Zebra GX430t hardware setup and driver installation',
      'Label media loading and auto-calibration',
      'QR code label template configuration in ZPL',
      'Asset tagging workflow: print → apply → scan → update',
      'CLEA app and hand scanner integration for real-time asset updates',
    ],
    curriculum: [
      { module: 'Module 1 — Zebra GX430t Overview & Setup', lessons: [
        'GX430t hardware tour: direct thermal printing, no ribbon required',
        'Unboxing and placement: USB connection, ventilation, desk positioning',
        'Driver installation via Zebra Setup Utilities on Windows',
        'Printer self-test: running a configuration label to verify setup',
        'Connecting to the network: shared printer vs. direct USB configuration',
      ]},
      { module: 'Module 2 — Label Media Loading & Calibration', lessons: [
        'Opening the media compartment: correct label threading path',
        'Loading the label roll: aligning guides to label width exactly',
        'Running auto-calibration: press feed button, printer detects label size',
        'Verifying calibration: test print confirms correct label positioning',
        'Common loading errors: roll shift, misaligned guides, incorrect media type',
      ]},
      { module: 'Module 3 — QR Code Label Template Design', lessons: [
        'ZPL (Zebra Programming Language) basics for IT asset labels',
        'Designing the SAP asset tag: SAP Equipment No. + Serial No. + QR code',
        'QR code data structure: encoding SAP number and serial in one code',
        'Label size and font selection for maximum scan reliability',
        'Testing the template: print and verify QR code scans correctly before going live',
      ]},
      { module: 'Module 4 — Asset Tagging Workflow', lessons: [
        'When to print: every device gets a tag before it leaves the IT desk',
        'Surface preparation: alcohol wipe, correct positioning by device type',
        'Label placement standards: laptops, phones, tablets, APs, network devices',
        'QR code verification after application: scan before handing over',
        'Handling server room equipment: rack units, switches, patch panels, APs',
      ]},
      { module: 'Module 5 — CLEA App & Scanner Integration', lessons: [
        'CLEA mobile app: point camera at QR code to open asset record instantly',
        'Updating asset status via CLEA: Stock → Assigned in one scan',
        'Hand scanner for bulk stock takes: scanning 50+ items in minutes',
        'Troubleshooting failed scans: faded print, partial damage, wrong orientation',
        'Monthly maintenance: printhead cleaning to prevent gradual quality degradation',
      ]},
    ],
  },
  {
  ...
  },
  {
    ...
    topics: [
      ...
      'Developer exclusion request workflow end-to-end',
      'Diagnosing high CPU on macOS with mdatp CLI',
      'Capturing and analysing Windows MpPerformanceRecording (.etl)',
      'Device re-onboarding after hardware repair',
      'L3 escalation: MDE Analyzer + Procmon + ETL artefact package',
      'Knowledge assessment',
    ],
    curriculum: [
      { module: 'Module 1 — MDE Architecture & Enterprise Management', lessons: [
        'What is Microsoft Defender for Endpoint — EDR, antivirus, threat intelligence in one platform',
        'MDE deployment models: standalone vs. MDM-managed (Intune) vs. SCCM co-managed',
        'Why centralised management via MDM is the enterprise standard',
        'Tamper Protection: what it blocks and why users cannot change settings',
        'Policy sync lifecycle: how MDM pushes MDE config and the propagation delay',
        'MDE licensing: Plan 1 vs. Plan 2 feature differences',
      ]},
      { module: 'Module 2 — Enterprise Performance Tuning', lessons: [
        'Why MDE can cause high CPU: real-time scanning, signature updates, cloud telemetry',
        'CPU throttling: ScanAvgCPULoadFactor — recommended range and how to apply',
        'Scan type selection: Quick Scan vs. Full Scan — enterprise default and scheduling',
        'Disabling scan-after-update: rationale and compliance trade-off',
        'Network drive exclusions: SMB/UNC paths and why real-time scanning causes latency',
        'Default developer directory exclusions: what is excluded and why',
        'Verifying applied exclusions on macOS: mdatp exclusion list',
      ]},
      { module: 'Module 3 — Developer Exclusion Request Workflow', lessons: [
        'Why developers need additional exclusions: compilation, containers, virtualisation',
        'Common exclusion types: IDE processes, build directories, Docker, Hyper-V',
        'Tamper Protection and why users cannot self-serve exclusions',
        'What to include in an exclusion request: hostname, role, specific paths',
        'IT support role: gathering information, routing to security/MDM team',
        'Policy group assignment and propagation time after approval',
      ]},
      { module: 'Module 4 — Troubleshooting MDE: macOS & Windows', lessons: [
        'macOS: identifying wdavdaemon high CPU in Activity Monitor',
        'macOS: mdatp health — sensor status, real-time protection, cloud connectivity',
        'macOS: mdatp exclusion list — confirming expected paths are present',
        'macOS: real-time scan monitoring with eslogger to identify scanned files',
        'Windows: capturing MpPerformanceRecording .etl during the performance issue',
        'Windows: Get-MpPerformanceReport — reading top-files output to find scan hot-spots',
        'Device re-onboarding after motherboard or logic board replacement',
      ]},
      { module: 'Module 5 — L3 Escalation & Advanced Scenarios', lessons: [
        'When to escalate: L1/L2 boundary vs. L3 backend security team',
        'MDE Analyzer: running the diagnostic tool and collecting the output bundle',
        'Procmon for MDE: capturing process activity to identify third-party conflicts',
        'The full L3 artefact package: MDE Analyzer + Procmon + .etl recording',
        'MDE not appearing active in portal: sensor health check and re-onboarding',
        'Post-escalation: what happens at L3 — cloud policy, exclusion backend, firmware-level issues',
      ]},
  ...
  },
  {
    ...
    instructor: 'Syed Waqas Tayyab',
    skills: ['Nexthink Amplify', 'Endpoint Analytics', 'Remote Actions', 'ITSM Integration', 'FCR Improvement', 'Proactive IT', 'MTTR Reduction'],
    description: 'Nexthink Amplify is a browser-based plugin that integrates real-time endpoint telemetry, AI diagnostics, and one-click remote actions directly into your ITSM ticketing tool. Learn to interpret live device data, run remote fixes, and improve First Contact Resolution at the service desk.',
    topics: [
      'What Nexthink Amplify is and how it integrates with ITSM',
      'Real-time endpoint telemetry: CPU, crashes, network, compliance',
      'One-click remote actions without remote desktop',
      'Guided diagnostics for FCR improvement at L1',
      'Proactive detection: crash spikes, compliance drift',
      'Knowledge assessment',
    ],
    curriculum: [
      { module: 'Module 1 — What is Nexthink Amplify?', lessons: ['Architecture: Collector → Infinity Platform → Amplify extension', 'How Amplify reads ticket data automatically', 'Installing the browser extension'] },
      { module: 'Module 2 — Real-Time Endpoint Telemetry', lessons: ['CPU, memory, boot time, app crashes', 'Network, Wi-Fi, disk, patch and compliance state'] },
      { module: 'Module 3 — One-Click Remote Actions', lessons: ['Clear cache, restart services, collect logs remotely', 'Automatic audit trail'] },
      { module: 'Module 4 — Guided Diagnostics & FCR', lessons: ['Slow laptop, Teams drops, post-re-image, recurring crashes', 'When to escalate vs. resolve'] },
      { module: 'Module 5 — Proactive IT', lessons: ['Crash spike and compliance drift detection', 'Building a proactive daily review workflow'] },
  ...
  },
  {
    ...
    img: '/snow-logo.png',
    instructor: 'Syed Waqas Tayyab',
    skills: ['ServiceNow', 'ITSM', 'SLA Management', 'KB Articles', 'Incident Management', 'ITIL v3'],
    description: 'Get up and running with ServiceNow from zero — navigate the interface, create and manage incidents, set up SLA rules, build your first knowledge base articles, and configure a live dashboard. Based on real daily operations handling 1,300+ interactions and 65+ catalog tasks per month.',
    topics: [
      'Navigating the ServiceNow interface: lists, forms, filters, and workspaces',
      'Creating and managing incidents: priority matrix, assignment, escalation',
      'Setting up SLA rules and targets: IRT/MPT, breach alerts',
      'Building knowledge base articles: templates, publishing, quality standards',
      'Configuring a live dashboard: KPI tiles, charts, and saved lists',
    ],
    curriculum: [
      { module: 'Module 1 — Getting Started with ServiceNow', lessons: [
        'What is ServiceNow and how it fits in enterprise ITSM',
        'The ServiceNow interface: Navigator, application menu, and workspaces',
        'Lists and forms: reading, filtering, sorting, and saving views',
        'Default lists vs. custom lists — creating your own filtered queue',
        'The CSM/FSM Configurable Workspace: overview for IT support agents',
        'Agent availability status: SAP IT, Chat, Calls, Walk-up — setting your channel',
      ]},
      { module: 'Module 2 — Incident Management', lessons: [
        'What is an Incident and when to raise one vs. a Service Request',
        'Creating an incident: required fields, category, subcategory, CI',
        'Priority matrix: P1 (Critical) → P4 (Low) — response and resolution times',
        'Assignment rules: auto-routing vs. manual assignment to groups',
        'Escalating an incident: when and how to escalate to a higher group',
        'Resolving and closing an incident: resolution notes and closure codes',
        'My Incidents list: filtering active, assigned, and group incidents',
      ]},
      { module: 'Module 3 — SLA Rules & Targets', lessons: [
        'What is an SLA in ServiceNow: IRT (Initial Response) vs. MPT (Max Processing)',
        'Reading SLA timers on a ticket: time remaining, breached, in-danger status',
        'SLA KPI dashboard tiles: Breached, In Danger, Resolved — what each means',
        'My SLAs Work list: finding tickets about to breach before they do',
        'SLA pause conditions: when the clock stops (e.g. awaiting user response)',
        'Best practice: act at 70% SLA consumed — not at 100%',
      ]},
      { module: 'Module 4 — Knowledge Base & Self-Service', lessons: [
        'What is a Knowledge Base article and why it reduces repeat tickets',
        'KB article structure: symptom → cause → resolution → escalation path',
        'Creating your first KB article from a resolved ticket',
        'Publishing and unpublishing articles: draft → review → live',
        'Linking a KB article to a ticket: how to attach during resolution',
        'Frequently used KBAs list: managing your personal KB shortcut list',
        'Self-service portal: how users find KB articles before raising tickets',
      ]},
      { module: 'Module 5 — Dashboard & Reporting', lessons: [
        'The "Happening Now" dashboard: building your live KPI view',
        'Adding KPI tiles: Incidents Resolved, Open Tasks, Breached SLAs',
        'Creating a saved list: filter → save as "My Assigned Incidents"',
        'Performance reports: tickets by category, volume by week, SLA % compliance',
        'Sharing a report or dashboard with your manager',
        'Setting your dashboard as the landing page for faster daily startup',
      ]},
    ],
  },
  {
    ...
    isFree: true, isPopular: true,
    img: 'https://images.unsplash.com/photo-1559526324-4b87b5e36e44?w=400&q=80&auto=format&fit=crop',
    instructor: 'Syed Waqas Tayyab',
  ...
  },
  {
    ...
    isFree: true, isPopular: true,
    img: 'https://images.unsplash.com/photo-1432888498266-38ffec3eaf0a?w=400&q=80&auto=format&fit=crop',
    instructor: 'Syed Waqas Tayyab',
  ...
  },
  {
    ...
    instructor: 'Syed Waqas Tayyab',
    skills: ['ServiceNow', 'SLA Design', 'Business Rules', 'KB Strategy', 'PowerBI', 'Auto-Assignment', 'Workflow Automation'],
    description: 'Build a production-ready ServiceNow environment from scratch — SLA frameworks (P1–P4), auto-assignment rules, business rules, KB architecture, workflow automation, and REST API integration. Based on 1,500+ tickets/year real operations experience with CSM/FSM Workspace and ITSM AI Copilot.',
    topics: [
      'SLA framework design: P1–P4 matrix with IRT/MPT targets and breach alerts',
      'Auto-assignment rules: category + location → correct team routing',
      'Business rules and client scripts: automating field updates and notifications',
      'KB strategy: 80+ article architecture, quality standards, FCR improvement',
      'REST API integration: reading and writing tickets via ServiceNow API',
    ],
    curriculum: [
      { module: 'Module 1 — ITSM Architecture & Process Design', lessons: [
        'ServiceNow ITSM architecture: CMDB, Incidents, Problems, Changes, Requests',
        'ITIL v3 alignment: Incident → Problem → Change lifecycle in ServiceNow',
        'Designing your service catalogue: categories, subcategories, offering hierarchy',
        'Assignment group design: naming conventions, membership, escalation paths',
        'SLA contract structure: IRT/MPT — reading and applying contract SLAs',
        'CSM/FSM Configurable Workspace setup: custom lists, KPI tiles, landing page',
      ]},
      { module: 'Module 2 — SLA Framework Design (P1–P4)', lessons: [
        'SLA record structure: target, condition, pause, reset, breach actions',
        'Designing the P1–P4 priority matrix: response times, resolution targets',
        'IRT (Initial Response Time) vs MPT (Maximum Processing Time) — configuring both',
        'SLA breach notifications: email alerts at 50%, 75%, 100% consumed',
        'Executive override rules: P1 auto-escalation to management on breach',
        'SLA dashboard: Breached, In Danger, Resolved KPI tiles — configuration',
        'Testing SLA rules: using test incidents to verify timers fire correctly',
      ]},
      { module: 'Module 3 — Auto-Assignment Rules & Routing', lessons: [
        'Assignment rules: condition builder — category + subcategory + location logic',
        'Routing by location: EMEA, MENA, KSA — regional routing configuration',
        'Routing by service offering: Meeting Room, Hardware, Mobile — separate queues',
        'Fallback assignment: what happens when no rule matches',
        'Testing assignment rules: dry-run with test tickets before go-live',
        'Auto-assignment in CSM/FSM Walk-up: how walk-up interactions get routed',
        'Monitoring misrouted tickets: report + corrective action workflow',
      ]},
      { module: 'Module 4 — Business Rules, KB & Workflow Automation', lessons: [
        'Business rules: when/then logic — auto-populate fields on ticket creation',
        'Client scripts: onChange, onLoad — dynamic form behaviour for agents',
        'Notification rules: email templates for ticket assignment, SLA breach, closure',
        'KB architecture: 80+ article strategy — symptom/cause/resolution template',
        'FCR improvement through KB: from <50% to 75%+ first contact resolution',
        'Quarterly KB review cycle: identifying stale articles, publishing updates',
        'Closing templates: standardised IT support closing messages with survey links',
      ]},
      { module: 'Module 5 — REST API, Reporting & PowerBI Integration', lessons: [
        'ServiceNow REST API: Table API — GET incidents by assignment group',
        'Authentication: session cookies vs. OAuth token for API access',
        'Python integration: polling ServiceNow every 5 minutes for SLA-at-risk tickets',
        'WhatsApp alert bot: Twilio + ServiceNow API — breach alert delivery',
        'PowerBI integration: connecting ServiceNow data to live KPI dashboards',
        'Monthly SLA report: automated generation — zero manual effort',
        'Metrics that matter: FCR %, SLA compliance %, ticket volume by category',
      ]},
    ],
    certificate: true,
  },
  {
    ...
    isFree: false, isPopular: false,
    img: 'https://images.unsplash.com/photo-1544197150-b99a580bb7a8?w=400&q=80&auto=format&fit=crop',
    instructor: 'Syed Waqas Tayyab',
  ...
  },
  {
    ...
    isFree: false, isPopular: true,
    img: 'https://images.unsplash.com/photo-1522199710521-72d69614c702?w=400&q=80&auto=format&fit=crop',
    instructor: 'Syed Waqas Tayyab',
            ...
            <div className="glass-card p-6 border border-accent-blue/15">
              <div className="flex flex-col sm:flex-row items-center sm:items-center gap-5">
                ...
                <div className="flex-1 text-center sm:text-left">
                  ...
                  <a href="/about" className="text-lg font-black text-white mb-0.5 hover:text-accent-blue transition-colors inline-flex items-center gap-1.5 group">Syed Waqas Tayyab <span className="text-[10px] text-gray-500 group-hover:text-accent-blue font-normal">View Profile →</span></a>
