...
const courses = [
  ...
  { id: 115,
    ...
    whatYouLearn: [
      ...
      'Assign pre-staged devices to new joiners and link to ServiceNow and enterprise asset management system records',
    ...
    curriculum: [
      ...
      { module: 'Module 5 — Power BI Asset Reporting & KPI Tracking', lessons: [
        ...
        'Missing Data report: identifying assets with incomplete enterprise asset management system records',
  ...
  { id: 116,
    ...
    whatYouLearn: [
      ...
      'Use ITSM AI Copilot to surface use cases and KB articles inside every ticket',
  ...
  { id: 123, title: 'Nexthink Amplify: Real-Time Endpoint Visibility for IT Support', category: 'Corporate IT Training', level: 'Intermediate', duration: '2h', students: '65+', rating: 4.9, isFree: false, isPopular: true, img: '/nexthink-logo.png', skills: ['Nexthink Amplify', 'Endpoint Analytics', 'Remote Actions', 'ITSM Integration', 'FCR Improvement', 'Proactive IT', 'MTTR Reduction', 'Digital Workplace'], description: 'Nexthink Amplify is a browser-based plugin that integrates real-time endpoint telemetry, AI diagnostics, and one-click remote actions directly into your ITSM ticketing tool. This course covers the full Amplify workflow — from understanding the architecture to using remote actions, interpreting telemetry, and improving First Contact Resolution at the service desk.', whatYouLearn: ['Understand how Nexthink Amplify integrates with your ITSM platform and surfaces endpoint data inside tickets', 'Interpret real-time device telemetry: CPU, memory, crashes, boot time, network, compliance', 'Use one-click remote actions to resolve common issues without remote desktop', 'Apply guided diagnostics to improve First Contact Resolution at L1', 'Identify proactive signals — crash spikes, compliance drift — before users raise tickets'], curriculum: [
    ...
    { module: 'Module 5 — Proactive IT', lessons: ['Login time degradation trends: identifying hardware model issues proactively', 'Compliance drift detection: flagging devices that have fallen out of policy', 'Network instability patterns: location-based AP fault identification', 'Building a proactive IT workflow: daily telemetry review and threshold alerts'] },
  ...
  { id: 124, title: 'IT Support Tier 1: Helpdesk & Service Desk Essentials', category: 'Corporate IT Training', level: 'Beginner', duration: '2h', students: '0+', rating: 5.0, isFree: false, img: 'https://images.unsplash.com/photo-1521737852567-6949f3f9f2b5?w=800&q=80&auto=format&fit=crop', skills: ['ServiceNow', 'Jira Service Management', 'Microsoft 365', 'Active Directory', 'Password Reset', 'Ticket Logging', 'SLA Management', 'ITIL v3', 'User Communication'], description: 'Master the foundations of enterprise IT support at Tier 1 — ticket logging, SOP-based triage, password resets, software installs, and professional user communication using the most widely deployed corporate IT tools: ServiceNow, Jira Service Management, Microsoft 365, and Azure AD. Based on real enterprise IT helpdesk operations handling 1,300+ interactions per year.', whatYouLearn: ['Log, categorise, and prioritise IT tickets correctly in ServiceNow and Jira Service Management', 'Handle the most common Tier 1 requests: password resets, MFA setup, software installs, basic hardware swaps', 'Read and act on SLA IRT/MPT timers before breach using the ITSM dashboard', 'Communicate professionally with users across walk-up, phone, chat, and email channels', 'Apply SOPs consistently and escalate correctly to Tier 2 with full documentation'], curriculum: [
    { module: 'Module 1 — The Role of Tier 1 IT Support', lessons: [
      'What Tier 1 owns vs. what it escalates — the ITIL incident management model',
      'The IT support pyramid: L1 (Helpdesk) → L2 (Desktop) → L3 (Engineering)',
      'Key performance metrics: FCR %, SLA compliance %, CSAT, ticket volume',
      'ITSM tools landscape: ServiceNow, Jira Service Management, Freshservice, Zendesk, BMC Helix',
      'Agent channels in ServiceNow CSM/FSM: Walk-up, Chat, Phone, Email — setting availability',
      'The ITIL v3 incident lifecycle: Log → Categorise → Prioritise → Assign → Resolve → Close',
    ]},
    { module: 'Module 2 — Ticket Logging & Triage in ServiceNow & Jira', lessons: [
      'Incident vs. Service Request — when to raise which and why it matters',
      'ServiceNow: creating an interaction, selecting category/subcategory, assigning CI',
      'Jira Service Management: creating a request, applying request type, setting priority',
      'Priority matrix P1–P4: response times, resolution targets, escalation triggers',
      'SLA timers: reading IRT (Initial Response) and MPT (Max Processing Time) in ServiceNow',
      'SLA dashboard tiles: Breached, In Danger, Resolved — acting at 70% consumed',
      'Closing codes and resolution notes — why quality closure data matters for KB articles',
    ]},
    { module: 'Module 3 — Common Tier 1 Resolutions', lessons: [
      'Password reset: Self-Service Password Reset (SSPR) vs. admin reset in Azure AD / Entra ID',
      'MFA setup and recovery: Microsoft Authenticator, SMS, RSA SecurID — step-by-step',
      'Software installation: Microsoft 365 Apps, Teams, Zoom — download, licence, and activation',
      'Printer issues: driver install, network printer mapping, toner/paper error handling',
      'Basic connectivity: Wi-Fi not connecting, VPN GlobalProtect first-time setup',
      'Microsoft 365 activation errors: account licence, shared computer activation, token reset',
      'Outlook profile issues: AutoDiscover failures, cached mode, OST file repair',
    ]},
    { module: 'Module 4 — Hardware Support & Device Basics', lessons: [
      'Device collection and return at the IT desk — inspection checklist and asset update',
      'Peripheral setup: monitor, keyboard, mouse, docking station, external display config',
      'Loaner device process: how to issue, track, and recover loan equipment',
      'Basic hardware triage: distinguishing software vs. hardware faults before escalation',
      'Updating the asset record in ITSM after a hardware swap or peripheral issue',
      'Mobile device enrolment basics: Company Portal, corporate Wi-Fi first-time setup',
    ]},
    { module: 'Module 5 — User Communication & Professionalism', lessons: [
      'Walk-up queue management: greeting, queue position, expectation setting',
      'Telephone support: active listening, clear instructions, call notes in ticket',
      'Chat support via ServiceNow or Microsoft Teams: response time, tone, documentation',
      'Closing templates: how to write a professional resolution message with survey link',
      'Escalation etiquette: what to include when handing off to Tier 2',
      'Handling frustrated users: de-escalation techniques and SLA commitments',
      'Building your personal KB shortlist: the 10 articles that save you daily',
    ]},
  ]},

  { id: 125, title: 'IT Support Tier 2: Desktop & Technical Support', category: 'Corporate IT Training', level: 'Intermediate', duration: '3h', students: '0+', rating: 5.0, isFree: false, img: 'https://images.unsplash.com/photo-1518770660439-4636190af475?w=800&q=80&auto=format&fit=crop', skills: ['Azure AD / Entra ID', 'Microsoft Intune', 'Jamf Pro', 'SCCM', 'ServiceNow', 'Jira', 'GlobalProtect VPN', '802.1X', 'Active Directory', 'Windows 11', 'macOS', 'Root Cause Analysis'], description: 'Handle escalated Tier 2 requests independently — Azure AD and Active Directory management, endpoint troubleshooting on Windows and macOS using Intune and Jamf, MDM compliance remediation, VPN and 802.1X network diagnosis, and advanced OS repairs. Based on real enterprise IT support at a large enterprise managing 200+ users across multiple offices.', whatYouLearn: ['Manage user accounts, groups, and licences in Azure AD / Entra ID and on-prem Active Directory', 'Troubleshoot Windows 11 endpoints using Intune, Autopilot, and SCCM', 'Diagnose and resolve macOS issues using Jamf Pro — enrolment, FileVault, Kerberos SSO', 'Resolve network escalations: GlobalProtect VPN, 802.1X Wi-Fi, NAC quarantine, DNS', 'Perform root-cause analysis on complex desktop issues and document findings in ServiceNow or Jira'], curriculum: [
    { module: 'Module 1 — Identity & Access Management at Tier 2', lessons: [
      'Azure AD / Entra ID architecture: users, groups, roles, licences, and conditional access',
      'Creating and modifying user accounts: UPN, display name, department, manager, location',
      'Group types: Security vs. M365 vs. Distribution — when to use which',
      'Licence assignment: Microsoft 365 E3/E5, individual service plans, group-based licensing',
      'MFA troubleshooting: re-registering Authenticator, number match issues, legacy auth blocks',
      'Conditional Access policy errors: non-compliant device, unsupported browser, named location',
      'On-prem Active Directory: OU structure, account properties, group policy basics, AD Connect sync',
      'Handling ITSM escalations in ServiceNow and Jira: assignment, notes, resolution documentation',
    ]},
    { module: 'Module 2 — Windows Endpoint Troubleshooting', lessons: [
      'Intune device compliance: reading compliance state, remediation workflow, re-sync trigger',
      'Autopilot troubleshooting: ESP failures, hybrid join errors, user-assigned vs. self-deploying',
      'SCCM software deployment: collection-based deployment, deployment status, logs (AppEnforce.log)',
      'Windows 11 upgrade failures: compatibility check, feature update staging, WUfB policies',
      'BitLocker issues: escrow key to Azure AD, recovery key entry, suspended state',
      'Event Viewer and reliability history: identifying recurring crashes and failed services',
      'Windows recovery options: Startup Repair, Reset This PC (keep files), clean reinstall via WinPE',
      'BIOS/UEFI: Secure Boot, TPM 2.0, boot order — common causes of provisioning failure',
    ]},
    { module: 'Module 3 — macOS Support Using Jamf Pro', lessons: [
      'Jamf Pro architecture: policies, smart groups, extension attributes, inventory',
      'ADE vs. Manual Device Enrolment — diagnosing enrolment failures and MDM profile issues',
      'FileVault: enforcing via policy, recovery key escrow, deferred enablement',
      'Kerberos SSO: certificate renewal via Jamf, corporate password sync failures — most common Mac ticket',
      'Self Service app portal: package deployment, app removal, compliance scripts',
      'macOS update compliance: nudge policies, managed deferrals, 30-day enforcement window',
      'T2 and Apple Silicon recovery: Recovery Mode reinstall, ASR restore, DFU mode',
      'GlobalProtect VPN + Conditional Access on macOS: registration, certificate, MFA requirements',
    ]},
    { module: 'Module 4 — Network & Connectivity Diagnosis', lessons: [
      'GlobalProtect VPN: client install, gateway selection, certificate errors, split-tunnel issues',
      '802.1X Wi-Fi authentication: TLS certificate chain verification, RADIUS errors, credential caching',
      'NAC quarantine (ForeScout/Cisco ISE): quarantine triggers, remediation steps, port re-enable',
      'DNS troubleshooting: nslookup, dig, flush cache, Cisco Umbrella DNS security blocks',
      'VLAN and IP addressing: DHCP lease issues, static IP conflicts, gateway unreachable',
      'Aruba Wireless troubleshooting: AP association, SSID hidden, BSSID roaming issues',
      'ISP and WAN: identifying local vs. upstream issue, circuit failover status check',
      'Escalation criteria: when to hand off to Tier 3 network — what to document first',
    ]},
    { module: 'Module 5 — Advanced Desktop Operations & Documentation', lessons: [
      'PC-to-PC migration: OneDrive Known Folder Move, Outlook signatures, browser bookmarks, desktop items',
      'OS reinstall procedure: backup check, clean install, Autopilot re-enrolment, data restore',
      'Application conflict troubleshooting: process isolation, DLL errors, compatibility mode',
      'Remote support tools: Microsoft Quick Assist, TeamViewer, Bomgar — corporate vs. personal use',
      'Writing a Tier 2 resolution note for ServiceNow/Jira: root cause, steps taken, time spent',
      'Creating a KB article from a Tier 2 resolution: when to write, what to include',
      'Escalation to Tier 3: what information to provide — logs, screenshots, steps already taken',
    ]},
  ]},

  { id: 126, title: 'IT Support Tier 3: System Engineering & Infrastructure', category: 'Corporate IT Training', level: 'Advanced', duration: '4h', students: '0+', rating: 5.0, isFree: false, img: 'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=800&q=80&auto=format&fit=crop', skills: ['Azure AD / Entra ID', 'Microsoft 365 Tenant Admin', 'Cisco IOS', 'Aruba Wi-Fi 6', 'HP ProLiant Servers', 'Microsoft Defender for Endpoint', 'Zero Trust', 'Conditional Access', 'PIM', 'ServiceNow Advanced', 'Jira', 'PowerBI', 'P1 Incident Response', 'Vendor Management'], description: 'Operate at the expert infrastructure level — enterprise server and core network administration, full Microsoft 365 tenant management, Zero Trust security architecture, infrastructure change projects, and P1 incident leadership. Based on real Tier 3 responsibilities managing enterprise IT across 3 offices and 200+ users, including a Zero Trust rollout that raised Microsoft Secure Score from 41% to 78%.', whatYouLearn: ['Administer HP ProLiant servers, Cisco switches, and Aruba Wi-Fi 6 infrastructure', 'Manage the full Microsoft 365 tenant: Exchange, Teams, SharePoint, Intune, Azure AD, Conditional Access, PIM', 'Design and implement Zero Trust security architecture from scratch', 'Lead infrastructure change projects with phased rollout, risk planning, and rollback procedures', 'Manage vendor escalations and lead P1 major incident bridge calls with full documentation'], curriculum: [
    { module: 'Module 1 — Enterprise Server & Core Network Administration', lessons: [
      'HP ProLiant server management: iLO, RAID configuration, firmware updates, hardware health monitoring',
      'Rack management: cabling standards, power redundancy, UPS sizing, cooling airflow',
      'Cisco IOS switches: VLAN creation, trunk/access port config, STP, PoE management, port mirroring',
      'Aruba Wi-Fi 6 deployment: AP placement, RF planning, Aruba Central management, rogue detection',
      'NAC administration (ForeScout/Cisco ISE): policy design, device profiling, quarantine automation',
      'ISP dual-path failover: BGP/static routing, failover testing, RTO measurement',
      'Change request process: RFC submission, impact assessment, approval chain, rollback plan',
      'Network documentation: VLAN register, IP address plan, rack diagram, SharePoint runbook',
    ]},
    { module: 'Module 2 — Microsoft 365 Tenant Management', lessons: [
      'Tenant architecture: Azure AD, Exchange Online, SharePoint, Teams, OneDrive — admin centre overview',
      'Exchange Online: accepted domains, connectors, transport rules, journal rules, retention policies',
      'Teams administration: policy packages, meeting policies, calling plans, Teams Rooms admin',
      'SharePoint governance: hub sites, content types, external sharing policy, DLP sensitivity labels',
      'Intune policy design: compliance policies, configuration profiles, app protection policies (MAM)',
      'Licence management: E3/E5 cost optimisation, group-based licensing, quarterly audit',
      'ServiceNow and Jira integration with M365: webhook notifications, ticket auto-creation from alerts',
      'Microsoft 365 health monitoring: Service Health dashboard, Message Centre, Admin app alerts',
    ]},
    { module: 'Module 3 — Zero Trust Security Architecture', lessons: [
      'Zero Trust principles: verify explicitly, least privilege, assume breach — applied to enterprise IT',
      'Conditional Access policy design: device compliance, named locations, risk-based sign-in, MFA requirements',
      'Legacy authentication blocking: identifying legacy auth in Azure AD sign-in logs, report-only mode first',
      'Privileged Identity Management (PIM): just-in-time access, approval workflows, audit trail',
      'Microsoft Defender for Endpoint: onboarding policy via Intune, Secure Score improvement roadmap',
      'DLP policies: sensitivity labels on SharePoint and email, endpoint DLP for Windows devices',
      'Microsoft Sentinel basics: data connectors, analytic rules, incident creation, SIEM alert triage',
      'Security incident response: Azure AD risky sign-in, compromised account playbook, evidence collection',
    ]},
    { module: 'Module 4 — Infrastructure Projects & Change Management', lessons: [
      'Project lifecycle for IT infrastructure: plan → design → procure → deploy → test → hand over',
      'Phased rollout strategy: IT pilot → low-risk users → standard → executives — lessons from 200+ user Intune migration',
      'SAP Ariba procurement: BOM creation, vendor selection, PR/PO approval chain, delivery tracking',
      'Weekend cutover planning: pre-flight checklist, communications plan, rollback criteria, go/no-go decision',
      'Post-implementation review (PIR): what worked, what failed, documentation updates',
      'PowerBI for infrastructure reporting: ServiceNow ticket trends, Intune compliance, Secure Score over time',
      'Jira for infrastructure projects: epic/story/task hierarchy, sprint planning, burndown tracking',
      'Documentation standards: network diagrams in Visio/draw.io, runbooks in SharePoint, change log',
    ]},
    { module: 'Module 5 — Vendor Management & P1 Incident Response', lessons: [
      'Microsoft Support: severity levels, Premier/Unified support, how to raise an effective P1 case',
      'Cisco TAC: case creation, collecting show commands and logs, PSIRT vulnerability tracking',
      'HP support: iLO case submission, hardware replacement SLA, on-site engineer coordination',
      'ServiceNow Major Incident process: P1 declaration, bridge call setup, stakeholder communication',
      'Jira escalation workflows: priority override, executive notification, SLA breach management',
      'P1 bridge call leadership: roles (incident commander, tech lead, comms), update cadence, resolution steps',
      'Root Cause Analysis (RCA): 5-whys methodology, contributing factors, permanent fix vs. workaround',
      'Vendor SLA management: tracking contracted response/resolution times, credit claims, escalation matrix',
    ]},
  ]},
