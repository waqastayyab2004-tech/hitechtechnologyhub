...
const courses = [
  ...
  },
  {
    ...
    description: 'Learn how to deploy, configure, and operate a Signifi Digital Storage Locker as a self-service IT Pickup Point — enabling employees to collect approved IT hardware 24/7 without IT staff involvement. Covers locker placement, SignifiVISION software, asset management integration, audit trail configuration, and daily operations. Based on a real deployment managing daily IT hardware collection.',
    topics: [
    ...
    curriculum: [
      { module: 'Module 1 — Smart Locker Fundamentals & Use Cases', lessons: [
        'What is a digital storage locker and how it differs from traditional asset handoff',
        'Signifi locker architecture: hardware (modular compartments, touch screen) + SignifiVISION software',
        'Use cases: 24/7 IT equipment access, quick purchase retrieval, secure storage, loan/swap',
        'How IT Pickup Points reduce manual handoff time and eliminate sign-out sheets',
        'Signifi at scale: 350M+ transactions/year, 45+ countries, 85+ API integrations',
        'Compliance advantage: authenticated digital audit trail vs. manual sign-out sheets',
      ]},
      { module: 'Module 2 — Deployment Planning & Installation', lessons: [
        'Placement strategy: lobby vs. IT room vs. floor — what drives the decision',
        'Physical requirements: floor space, power supply, network connectivity (LAN/Wi-Fi)',
        'Compartment size planning: small (phones/accessories), medium (tablets), large (laptops/MacBooks)',
        'Branding and signage: "IT Pickup Point" label, wayfinding, QR code instructions on-screen',
        'Network configuration: locker IP assignment, firewall rules for SignifiVISION cloud connectivity',
        'First-time setup: physical commissioning, screen boot, admin login, locker ID registration',
      ]},
      { module: 'Module 3 — SignifiVISION Software Administration', lessons: [
        'SignifiVISION dashboard: overview, real-time locker status, compartment occupancy',
        'User management: adding employees, assigning access levels, linking to corporate directory',
        'Compartment assignment: assigning a specific locker slot to a user or order',
        'Notification configuration: email/SMS templates for "your item is ready to collect"',
        'Personalised alerts: locker full, item not collected after X hours, access failure',
        'Reporting: transaction history, user access log, asset dwell time, utilisation rate',
        'Centralised management: managing multiple lockers across multiple locations from one portal',
      ]},
      { module: 'Module 4 — Asset Management Integration', lessons: [
        'End-to-end workflow: SAP Ariba order approved → IT loads locker → employee notified → collects → CLEA record updated',
        'ServiceNow integration: fulfilment ticket automatically linked to locker assignment number',
        'CLEA/SAP ISP update on collection: asset status changes from "In Stock" to "Assigned" automatically',
        'Return workflow: employee deposits used/broken device → locker records return → ticket auto-raised',
        'API integration: Signifi 85+ API integrations — connecting to SAP, ServiceNow, and MDM tools',
        'Audit trail: every collection event — user ID, timestamp, compartment, asset serial — stored in SignifiVISION',
      ]},
      { module: 'Module 5 — Daily Operations & Troubleshooting', lessons: [
        'Daily checklist: load approved orders, verify compartment availability, check notification queue',
        'Employee experience walkthrough: receive notification → arrive at locker → authenticate → collect',
        'Authentication methods: PIN code, QR code, badge tap, biometric (model dependent)',
        'Common issues: compartment stuck, employee locked out, notification not received — resolution steps',
        'UV sanitisation cycle: when to run, how to verify completion, compliance logging',
        'Escalation path: locker hardware fault → Signifi support → temporary manual handoff protocol',
        'Monthly maintenance: clean touch screen, check power connections, review access logs for anomalies',
      ]},
    ],
  },
  {
