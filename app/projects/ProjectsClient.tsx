...
const projects: Project[] = [
  ...
  },
  {
    ...
    tags: ['Zebra GX430t', 'QR Code', 'Asset Tagging', 'CLEA App', 'Label Printing', 'IT Asset Management'],
    pmDetails: {
      scope: 'Deploy and operationalise the Zebra GX430t label printer for QR code asset tagging across all enterprise IT equipment — from end-user devices to server room infrastructure.',
      objectives: [
      ...
      stakeholders: 'IT Team, Asset Management Team, End Users, IT Manager',
      ...
      toolsUsed: ['Zebra GX430t', 'Zebra Setup Utilities', 'CLEA App', 'QR/Barcode Scanners', 'IT Asset Management System', 'ServiceNow'],
      learnings: [
  ...
  {
    icon: Tag, group: 'Printer Management',
    title: 'Zebra ZT411 RFID Printer — Enterprise IT Asset Tag Deployment', subtitle: 'Zebra ZT411R · 203dpi · ZPL · RFID · Thermal Transfer · Network-Connected',
    status: 'Live', category: 'IT Asset Management · RFID · Print Infrastructure',
    description: 'Full end-to-end deployment of the Zebra ZT411R industrial RFID printer at the corporate IT office — from unboxing and physical installation through thermal transfer media loading, manual media calibration, RFID calibration, network registration via DHCP, and print server configuration. Printer is now fully operational with Status: READY, connected to the corporate network via wired Ethernet, and integrated into the IT asset tagging workflow. Used daily to print RFID-enabled asset tags for all IT hardware.',
    highlights: [
      'Installed Zebra ZT411R (203dpi, ZPL, Thermal Transfer) — Status: READY, wired Ethernet connected',
      'Loaded RFID inlay media and ribbon — configured thermal-transfer print mode at standard darkness and print speed',
      'Performed manual media calibration: held Pause + Cancel for sensor teach cycle, removed ribbon during calibration, verified single label feed on completion',
      'Performed RFID calibration via Menu → RFID → RFID Calibrate → Start Calibration — selected correct country/region; process ran for 5+ minutes confirming full calibration',
      'Set Head Close Action to No Motion (Settings → Head Close Action) — prevents label waste on printhead close',
      'Disabled sleep mode / Power Save to prevent network port deactivation by the network team during inactivity',
      'Connected to corporate network via Ethernet (DHCP) — IP assigned dynamically; print server accessible via browser at printer IP address',
      'Configured ZebraNet internal wired print server — bidirectional communication enabled, link status Good at 100Mb/s',
      'SNMP alert configured: Cold Start broadcast to network segment for monitoring',
      'Printer web interface accessible: View/Modify Settings, RFID Log, Alert Setup, Directory Listing, Print Server Settings all verified',
      'Memory verified: ~63MB onboard flash free, ~7.6MB RAM free — full font library loaded (ZPL fonts, TTF, barcode codepages)',
      'RFID firmware: ZBI 2.1, RFID Reader active, RFID Region set to correct country, RFID status READY',
      'Firmware: V92.21.34Z — current production release; RTC date/time set correctly',
      'Printer registered on network by the central Printer Team — MAC address confirmed, hostname assigned',
      'Integrated with IT asset management workflow: every device tagged before assignment or dispatch',
    ],
    tags: ['Zebra ZT411R', 'ZPL', 'RFID', 'Thermal Transfer', 'Asset Tagging', 'ZebraNet Print Server', 'DHCP', 'Network Printing', 'EU RED', 'Label Printing', 'IT Asset Management'],
    pmDetails: {
      scope: 'Deploy and operationalise the Zebra ZT411R RFID printer — covering physical installation, media and ribbon loading, manual and RFID calibration, network registration, print server configuration, and integration into the IT asset tagging workflow.',
      objectives: [
        'Install and configure Zebra ZT411R at the IT office — Status READY before first production print',
        'Complete manual media calibration to ensure single-label feed accuracy',
        'Complete RFID calibration with correct regional settings for compliant RF operation',
        'Connect to corporate network via DHCP and register with the Printer Team',
        'Configure ZebraNet print server and verify bidirectional communication',
        'Disable sleep mode to prevent network port deactivation',
        'Enable RFID asset tagging for all IT hardware entering and leaving the office',
      ],
      stakeholders: 'IT team, Global RFID Programme Team, Network Team (port activation + DHCP), Central Printer Team (network registration)',
      timeline: 'July 2026',
      outcomes: [
        'Zebra ZT411R fully operational — Status READY, printing RFID asset tags in production',
        'Manual media calibration complete — single label feed verified, no waste on printhead close',
        'RFID calibration complete — correct region selected, full 5-minute calibration cycle completed, RFID status READY',
        'Network connected via wired Ethernet — DHCP IP assigned, print server accessible via browser',
        'ZebraNet print server configured — bidirectional communication enabled, SNMP alert active',
        'Sleep mode disabled — network port remains active during idle periods',
        'All new IT assets tagged before assignment — full RFID audit trail in place',
      ],
      skillsApplied: ['RFID Technology', 'Hardware Deployment', 'Printer Configuration', 'Network Administration', 'IT Asset Management', 'ZPL/ZebraNet', 'Compliance & Security'],
      toolsUsed: ['Zebra ZT411R RFID Printer', 'ZebraNet Internal Wired Print Server', 'Zebra Printer Web Interface', 'DHCP Network', 'SNMP Monitoring', 'RFID Inlay Media', 'Thermal Transfer Ribbon', 'ServiceNow'],
      learnings: [
        'Manual calibration (Pause + Cancel) requires removing the ribbon at the prompt — failure to do so prevents the sensor from reading the label backing correctly',
        'RFID calibration must run the full cycle (5+ minutes) — if it finishes quickly, it has not completed and must be re-run',
        'Set Head Close Action to No Motion before production use — default "Feed" on close wastes labels every time the printhead is opened',
        'Disable Power Save / sleep mode before network registration — network teams can disable idle switch ports, cutting the printer off the network silently',
        'Contact the Printer Team for network registration before any print queue setup — they assign the hostname and DHCP reservation; a dynamic IP change after setup breaks all connected queues',
        'RFID inlay alignment is critical — even a small offset causes consistent void tags; verify alignment with a test RFID write before loading the full roll',
        'The printer web interface (IP in browser) is the fastest way to verify full configuration — check Status READY, RFID Status READY, and print server link status in one view',
      ],
    },
  },
  {
  ...
  },
  {
    ...
    description: 'Full-stack Flask web app replacing Excel-based IT asset tracking. Manages 1,500–2,000+ active assets across MENA — modelled on enterprise asset ERP workflows. Includes AI chat widget for natural language queries.',
  ...
  },
  {
    ...
    highlights: [
      ...
      'ITSM AI Copilot: AI-powered assistant for use case search and resolution suggestions within tickets',
      ...
      'Closing templates: standardised IT support closing messages with satisfaction survey links',
    ],
    tags: ['ServiceNow CSM/FSM', 'ITSM AI Copilot', 'ITIL v3', 'SLA Management', 'Walk-up Queue', 'Catalog Tasks', 'Incident Management'],
    pmDetails: {
      ...
      objectives: [
        ...
        'Use ITSM AI Copilot to accelerate resolution and improve first-contact resolution rate',
      ...
      outcomes: [
        ...
        'ITSM AI Copilot integrated into daily resolution workflow',
      ...
      skillsApplied: [
        ...
        'AI-Assisted ITSM (ITSM AI Copilot)',
      ...
      toolsUsed: [
        ...
        'ITSM AI Copilot (AI resolution assistant)',
      ...
      learnings: [
        ...
        'ITSM AI Copilot reduces average handle time by surfacing relevant KB articles and use cases within the ticket — eliminating the search step that consumes 2–3 minutes per ticket',
  ...
  },
  {
    ...
    tags: ['ITSM AI Copilot', 'ServiceNow CSM/FSM', 'Walk-up Queue', 'ITIL v3', 'Catalog Tasks', 'Hardware Lifecycle', 'AI-Assisted ITSM'],
    pmDetails: {
      scope: 'Operate daily IT support using ServiceNow ITSM AI Copilot within the CSM/FSM workspace — managing the full range of IT support interactions, hardware lifecycle tasks, and onboarding workflows with AI assistance.',
      objectives: [
        'Use ITSM AI Copilot to accelerate every ticket resolution — zero manual KB search',
      ...
      outcomes: [
        'ITSM AI Copilot actively used in every interaction — surfacing use cases in real time',
      ...
      learnings: [
        'ITSM AI Copilot is most effective when the ticket description is specific — vague descriptions return generic results; training users to describe symptoms precisely improves AI suggestion quality',
  ...
  },
  {
    ...
    highlights: [
      ...
      'Credentials managed via corporate password vault — no hard-coded or printed passwords',
      'Queue display auto-refreshes every 15 min via ServiceNow Walk-up URL playlist',
    ],
    tags: ['ServiceNow Walk-up', 'Jamf MDM', 'Apple ADE', 'Microsoft Intune', 'Digital Signage', 'iPad', 'Queue Management', 'corporate password vault'],
    pmDetails: {
      ...
      objectives: [
        ...
        'Ensure secure credential management via corporate password vault — no printed passwords',
      ...
      outcomes: [
        ...
        'Zero credential exposure — all service account passwords in corporate password vault',
      ...
      learnings: [
        ...
        'Retrieving service account passwords from corporate password vault every time (not printing them) is essential — the .docx credential backup is a security risk to eliminate',
  ...
  },
  {
    ...
    highlights: [
      ...
      'On-boarding module: assign pre-staged devices to new joiners — links to ServiceNow ticket and enterprise asset ERP record',
      ...
      'Pool Stock: IT Storage Room — 89 pool devices across 6 categories live tracked',
    ...
    tags: ['SAP BTP', 'CLEA App', 'QR Asset Scanning', 'enterprise asset management system', 'Power BI', 'Onboarding', 'Offboarding', 'IT Asset Management'],
    pmDetails: {
      ...
      objectives: [
        'Maintain 100% accurate pool stock records via monthly QR scan of all 89 devices in IT storage room',
      ...
      outcomes: [
        '89 pool devices accurately tracked in IT storage room across 6 categories',
        ...
        'All on-boarding device assignments linked to enterprise asset management system, ServiceNow, and CLEA for full audit trail',
      ...
      toolsUsed: [
        ...
        'SAP BTP Cloud Foundry (European data centre region)',
      ],
      learnings: [
  ...
  },
  {
    ...
    title: 'Bank ATM & Branch Security System', subtitle: 'Banque Saudi Fransi · enterprise access control system · ATM security system · 24/7',
...

  {
    ...
    pmDetails: {
      ...
      toolsUsed: [
        ...
        'Asset management system (CLEA / enterprise asset management system)',
  ...
  },
  {
    ...
    highlights: [
      ...
      'internal technology wiki — maintained documentation for all approved technology categories',
  ...
  },
  {
    ...
    pmDetails: {
      ...
      outcomes: [
        ...
        'Asset record in CLEA/enterprise asset management system updated automatically on collection — no manual data entry',
      ...
      toolsUsed: [
        ...
        'enterprise asset management system — asset record update on collection',
  ...
  },
  {
    ...
    status: 'Live', category: 'Cybersecurity · Endpoint Security',
    description: 'End-to-end operational ownership of Microsoft Defender for Endpoint (MDE) across a multi-OS enterprise fleet — covering centralised policy management via MDM, performance tuning, developer exclusion workflows, L1/L2 troubleshooting, and L3 escalation. Hands-on experience with both Windows and macOS deployment patterns, real-time scanning diagnostics, and enterprise support ticketing for exclusion requests.',
    highlights: [
      'Centralised MDE policy management via MDM (Intune) — Tamper Protection enforced, no local overrides permitted',
      'Performance tuning: CPU throttling limits, quick-scan scheduling, network drive exclusion to prevent latency',
      'Default developer exclusions pre-applied: IDE processes, build tool directories, container runtimes, and virtualisation',
      'Exclusion request workflow: raised via IT support ticket with device hostname, user role, and justification — approved and applied within ~24h policy sync',
      'macOS: real-time scanning diagnostics via mdatp CLI — identify high-CPU processes, verify exclusion list, check sensor status',
      'Windows: MpPerformanceRecording (.etl) analysis — identify top scanned files/processes causing user-reported slowdowns',
      'L3 escalation path: MDE Analyzer logs + Procmon + ETL recording submitted to backend security team',
      'Device re-onboarding after hardware changes (motherboard replacement, logic board swap) — re-run onboarding profile via MDM',
      'Common scenario triage: wdavdaemon high CPU, IDE slow startup, MDE not reporting in portal, post-repair onboarding gaps',
    ],
    tags: ['Microsoft Defender for Endpoint', 'EDR', 'Microsoft Intune', 'macOS Security', 'Windows Security', 'Endpoint Protection', 'Tamper Protection', 'Performance Tuning', 'Exclusion Management'],
    pmDetails: {
      scope: 'Operational support and policy management for Microsoft Defender for Endpoint across a managed enterprise device fleet — Windows and macOS. Covers day-to-day user support, exclusion request handling, performance diagnostics, and escalation to the backend security team.',
      objectives: [
        'Maintain MDE active and reporting on 100% of managed endpoints',
        'Resolve MDE-related performance complaints within agreed SLA',
        'Process developer exclusion requests with correct approvals and documentation',
        'Ensure device re-onboarding after hardware replacements causes zero security gaps',
        'Escalate unresolved EDR issues with full diagnostic artefacts to L3',
      ],
      stakeholders: 'End users (performance), developers (exclusion requests), IT Security (compliance), IT Manager (SLA), L3 backend security team (escalations)',
      timeline: 'Ongoing — daily support operations',
      outcomes: [
        'MDE active on 100% of managed endpoints — confirmed via MDM compliance dashboard',
        'Developer exclusion requests processed within 24h policy sync window',
        'High-CPU MDE complaints resolved at L1/L2 using mdatp diagnostics and ETL analysis',
        'Zero security gaps from post-repair device re-onboarding — MDM auto-reapplies profile',
        'L3 escalations fully documented — MDE Analyzer + Procmon + ETL always included',
      ],
      skillsApplied: ['Endpoint Security Operations', 'MDM Policy Administration', 'Cybersecurity Troubleshooting', 'Performance Diagnostics', 'Security Incident Triage', 'IT Support Escalation Management'],
      toolsUsed: ['Microsoft Defender for Endpoint', 'Microsoft Intune', 'mdatp CLI (macOS)', 'MpPerformanceRecording (Windows)', 'Procmon', 'MDE Analyzer', 'PowerShell', 'IT Service Management Portal'],
      learnings: [
        'Centralised Tamper Protection is non-negotiable — user-applied exclusions create untracked attack surface; the ticket workflow is the correct gate',
        'A pre-built developer exclusion policy group dramatically reduces repetitive ticket volume — onboarding developers into the right policy group at hire time prevents most slowdown complaints',
        'ETL performance recordings are the fastest way to identify exactly which files MDE is scanning — guessing based on user reports wastes hours',
        'Device re-onboarding after hardware replacement is a consistently missed step — a walkup checklist that includes "check MDE portal" after any motherboard/logic board swap closes this gap',
      ],
    },
  },
]
