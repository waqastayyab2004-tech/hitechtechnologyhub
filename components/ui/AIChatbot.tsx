...
/* ── Fallback keyword responses when API is offline ── */
...
export default function AIChatbot() {
  ...
  const send = async () => {
    ...
    try {
      ...
      /* API unreachable — use smart fallback */
      setAiMode('offline')
      await new Promise(r => setTimeout(r, 600))
      setTyping(false)
      setMessages(m => [...m, { role: 'bot', text: getFallbackReply(text) }])
    }
  }
    ...
    <>
      ...
      <div className="fixed bottom-6 left-6 z-50 flex flex-col items-center gap-2">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className="relative w-18 h-18 rounded-full shadow-[0_0_35px_rgba(59,130,246,0.7)] overflow-hidden border-[3px] border-accent-blue/80 bg-dark-800"
                  ...
                  {typing && (
                    <div className="flex gap-2">
                      ...
                      <div className="bg-dark-700 border border-white/8 px-4 py-3 rounded-2xl rounded-tl-none flex items-center gap-1">
                        {[0,1,2].map(i => (
                          <span key={i} className="w-1.5 h-1.5 bg-accent-blue rounded-full animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />
                        ))}
                      </div>
                    </div>
                  )}
...
/* ── Robot Avatar — uses the actual AI robot image ── */
